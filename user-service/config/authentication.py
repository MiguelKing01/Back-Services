...
class FirebaseAuthentication(BaseAuthentication):
    ...
    def authenticate(self, request):
        ...
        # Intentar obtener el modelo Usuario de la BD si existe con el firebase_uid
        try:
            from usuarios.models import Usuario
            user_db = Usuario.objects.filter(firebase_uid=uid).first()
            if user_db:
                user_db.claims = decoded_token
                return (user_db, decoded_token)
        except Exception:
            pass
